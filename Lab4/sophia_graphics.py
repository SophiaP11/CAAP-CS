...
def draw(pallet, pixels):
    for i in range(len(pixels)):
        for j in range(len(pixels[i])):
            color= pallet[pixels[i][j]] #assigns the color value of the pixel
            mypen.color(color) #changes the color of the pen to the correct color
            box(boxsize) #draws the box
            myPen.forward(boxsize) #moves turtle 1 pixel right
        myPen.down(boxsize)#move 1 down
        #move 1 len(line) left
